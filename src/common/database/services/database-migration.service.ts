...
import { MigrationRepository } from '../repositories/repository/migration.repository';
import { MigrationEntity } from '../repositories/entities/migration.entity';
...
export class MigrationService {
  ...
  async findOneByVersion(version: string): Promise<MigrationEntity> {
    const migration = await this.migrationRepository.findByCondition({
  ...
  async loadMigrationFiles(dir: string): Promise<MigrationFile[]> {
    ...
    for (const file of files) {
      ...
      try {
        ...
        this.logger.warn(
          `Error parsing migration file: ${file} | ${e.message}`,
        );
      }
    }
  ...
  private parse(filename: string, content: string): MigrationFile {
    ...
    migrationFile.version = versionString;
    migrationFile.script = filename;
    migrationFile.description = match.groups.description;
