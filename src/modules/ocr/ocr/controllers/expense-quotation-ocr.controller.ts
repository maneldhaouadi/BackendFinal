...
import { OcrProcessResponse } from '../dtos/expense-quotation-ocr-result.dto';

@ApiTags('Expense Quotation OCR Processing')
@Controller('expense-quotation-ocr')
export class ExpenseQuotationOcrController {
  private readonly logger = new Logger(ExpenseQuotationOcrController.name);
  
  constructor(private readonly ocrService: ExpenseQuotationOcrService) {}

  @Post('process')
  @UseInterceptors(
    FileInterceptor('file', {
      storage: diskStorage({
        destination: './uploads/expense-quotation-ocr',
  ...
  @ApiOperation({
    summary: 'Process expense quotation document with OCR',
    description: 'Extracts structured data from expense quotation using OCR and AI processing',
  })
  @ApiResponse({
    status: 200,
  ...
  async cleanupFile(@Param('filename') filename: string) {
    const filePath = `./uploads/expense-quotation-ocr/${filename}`;
